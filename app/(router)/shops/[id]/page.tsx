...
const ShopsDetail = ({ params, searchParams }: Props) => {
  ...
  const queryDetailItem = (key: any) =>
    setStateDetailItem((prev) => ({ ...prev, ...key }));
    ...
    if (params?.id) {
      ...
      queryDetailItem({
        dataDetail: {
          ...dataDetail,
          imageSlider: {
            image: Array.isArray(dataDetail?.list_images)
              ? dataDetail?.list_images
              : [dataDetail?.image] || [],
            imageThumb: Array.isArray(dataDetail?.list_images)
              ? dataDetail?.list_images
              : [dataDetail?.image] || [],
          },
        },
      ...
      <div className="sticky top-0 z-[999] bg-white ">
        <div className="custom-container-child  flex items-center justify-between gap-2  py-2">
          <div className=" flex items-center gap-1">
            <div className="size-10">
              <IoIosArrowRoundBack
                onClick={() => router.back()}
                size={22}
                className="size-full cursor-pointer text-[#E73C2A]"
              />
            </div>
          ...
          <div
            ...
            className="hover:bg-[#E73C2A]/30 transition-all duration-150 cursor-pointer ease-linear bg-[#E73C2A]/10 rounded-xl group p-3 relative"
          >
            <div
        ...
        <div className="flex mb-2 h-[calc(100dvh_-_60px)] flex-col gap-1  overflow-y-auto ">
          <div className="custom-container-child">
            <Swiper
              ...
              className="mySwiper2"
            >
              {stateDetailItem.dataDetail?.imageSlider?.image?.map(
                (item, index) => (
                  <SwiperSlide
                    key={index}
                    className="min-h-[250px] h-[250px] max-h-[250px] cursor-pointer"
                  >
                    <Image
                      src={item}
                      alt=""
          ...
          <div className=" bg-white shadow-[0_0_5px_rgba(0,0,0,0.1)]">
            <div className="flex flex-col gap-2 custom-container-child py-2">
              <h1 className="text-[#545454] text-base font-semibold leading-1">
                {stateDetailItem.dataDetail?.name}
              </h1>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-gray-400 font-normal text-sm leading-1">
                  (0.0 đánh giá)
                </h1>
                <h1 className="text-gray-400 font-normal text-sm leading-1">
                  Đã bán: {FormatNumberDot(stateDetailItem.dataDetail?.count)}
                </h1>
              </div>
              <h1 className="text-[#545454] font-medium text-lg leading-1">
                {FormatNumberDot(stateDetailItem.dataDetail?.price)} vnđ
              </h1>
            </div>
          </div>
          <div className="bg-white shadow-[0_0_5px_rgba(0,0,0,0.1)]">
            <div className="custom-container-child py-2">
              <h1 className="text-gray-500 font-normal text-base leading-1 mb-2">
                Các loại hàng
              </h1>
              <Swiper
                ...
                className="mySwiper"
              >
                {stateDetailItem.dataDetail?.imageSlider?.imageThumb?.map(
                  (item, index) => {
                    return (
                      <>
                        <SwiperSlide key={index} className="size-[65px]">
                          <div
                            ...
                            className={`size-[65px] cursor-pointer rounded-lg transition-all duration-150 ease-linear ${
                              selectedSlide === index
                                ? "border-2 border-[#E73C2A]"
          ...
          <div className="bg-white shadow-[0_0_5px_rgba(0,0,0,0.1)] py-2 flex flex-col gap-2">
            <h1 className="text-[#E73C2A] font-semibold text-lg text-center border-b-2 border-[#E73C2A] pb-2">
              Giới thiệu
            </h1>
            <div
              className="custom-container-child text-[15px]  font-normal"
              dangerouslySetInnerHTML={{
                __html: stateDetailItem.dataDetail?.content,
              }}
            ></div>
          </div>
        </div>
        <div className="grid grid-cols-2 items-center justify-center h-[48px]">
          ...
          >
            <div className="">
              ...
              <h1 className="text-sm text-white font-medium h-full">
                Thêm vào giỏ hàng
              </h1>
            </div>
          </div>
          <div
            ...
            className="text-sm bg-[#E73C2A] transition-all duration-150 ease-linear hover:bg-[#E73C2A]/80 h-full flex items-center justify-center text-white cursor-pointer"
          >
            Mua ngay
          </div>
        </div>
